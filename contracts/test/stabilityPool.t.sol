...
contract SPTest is DevTestSetup {
    ...
    function testLiquidationWithLowPDoesNotDecreaseP_Cheat_Fuzz(uint256 _cheatP, uint256 _surplus) public {
        ...
        vm.store(
            ...
            bytes32(uint256(P_SLOT)), // 60th storage slot where P is stored
            bytes32(uint256(_cheatP))
        );

        // Confirm that storage slot 55 is set
        uint256 storedVal = uint256(vm.load(address(stabilityPool), bytes32(uint256(P_SLOT))));
        assertEq(storedVal, _cheatP, "value of slot 55 is not set");
    ...
    function testLiquidationsWithLowPAllowFurtherRewardsForAllFreshDepositors_Cheat_Fuzz(
        ...
        vm.store(
            address(stabilityPool),
            bytes32(uint256(P_SLOT)), // 55th storage slot where P is stored
            bytes32(uint256(_cheatP))
        );
